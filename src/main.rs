...
use image::{open, DynamicImage, ImageBuffer, Rgb, RgbImage, RgbaImage};
...
impl Particle {
    ...
    fn moveParticle(
            ...
            && self.isDead == false
        {
            ...
            self.caryCapacity = (speed / (1.2 * 9.81 * 256 as f32).sqrt() * (self.lifetime as f32 / self.maxLifetime as f32)) * 4.0;
...
fn main() {
    ...
    for _l in 0..20 {
        ...
        let ammount_particles = 100000;//100000
      ...
      //  let sumDispote: f32 = buffer_disposite.iter().sum();
        ...
        let pathNumber = _l.to_string().to_owned();
        let format = ".png";
        let  mut fileName= "testing".to_owned();
        fileName.push_str(&pathNumber);
...
fn blur_buffer(array_to_blur:Vec<f32>)->Vec<f32>{
    ...
    let mut blur_buffer = ImageBuffer::new(512,512);
        ...
        image::imageops::blur(&blur_buffer, 100.0);
